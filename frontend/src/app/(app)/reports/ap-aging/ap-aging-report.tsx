...
import { useApAging } from "@/lib/queries/reports";

const BUCKETS = ["current", "1-30", "31-60", "61-90", "91+"] as const;

const BUCKET_KEYS: Record<(typeof BUCKETS)[number], string> = {
  current: "reports.apAging.buckets.current",
  "1-30": "reports.apAging.buckets.b1to30",
  "31-60": "reports.apAging.buckets.b31to60",
  "61-90": "reports.apAging.buckets.b61to90",
  "91+": "reports.apAging.buckets.b91plus",
};
...
export function ApAgingReport() {
  ...
  const { data, isLoading } = useApAging(asOf);
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
              <tr className="text-left">
                <Th>{t("reports.apAging.vendor")}</Th>
                {BUCKETS.map((b) => (
